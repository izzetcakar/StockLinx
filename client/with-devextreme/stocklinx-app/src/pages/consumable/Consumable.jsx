...
const Consumable = ({ Id }) => {
  ...
  const consumables = useSelector((state) => state.product.consumables);
  ...
  const consumableData = new DataSource({
    store: {
      type: "array",
      data: JSON.parse(JSON.stringify(consumables)),
      // key: "id",
    },
  });
  const onExporting = (e) => {
  ...
  const onRowUpdating = async (e) => {
    // const res = await dispatch(updateConsumable({ ...e.oldData, ...e.newData }));
    // notifyError(res);
    // dispatch(getConsumables());
  };
  const onRowInserting = async (e) => {
    // const res = await dispatch(createConsumable({ ...e.data, checklistId: Id }));
    // notifyError(res);
    // dispatch(getConsumables());
  };
  const onRowRemoving = async (e) => {
    // const res = await dispatch(removeConsumable(e.data.id));
    // notifyError(res);
    // dispatch(getConsumables());
  };
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        >
          ...
          <Form>
            <Item itemType="group" colCount={2} colSpan={2}>
              ...
              <Item dataField="modelNo" />
              <Item dataField="itemNo" />
              <Item dataField="tag" />
              <Item
                dataField="note"
                itemType="dxTextArea"
        ...
        <Column dataField="modelNo" caption="Model No" dataType="string" />
        <Column dataField="itemNo" caption="Item No" dataType="string" />
