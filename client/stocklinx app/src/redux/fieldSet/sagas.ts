...
function* fetchFieldSetsSaga() {
  ...
  try {
    const { data }: IResponse = yield call(fieldSetRequests.getAll);
    yield put(
...
function* removeFieldSetSaga(action: RemoveFieldSetRequest) {
  yield put(genericActions.increaseLoading());
  try {
    yield call(fieldSetRequests.remove, action.payload.id);
...
function* removeRangeFieldSetSaga(action: RemoveRangeFieldSetRequest) {
  yield put(genericActions.increaseLoading());
  try {
    yield call(fieldSetRequests.removeRange, action.payload.ids);
